...
public class Identicon: IconGenerator {
    ...
    public func icon(from number: UInt32, size: CGSize) -> CGImage {
        ...
        context.saveGState()
        let background = CGColor.color(from: 0xFFFF)
        context.setFillColor(background)
        context.fill(CGRect(origin:CGPoint.zero, size:normalizedSize))
        ...
        return scaledImage.paint(over:CGColor.color(from: UInt16(number & 0xFFFF)))!
    }
